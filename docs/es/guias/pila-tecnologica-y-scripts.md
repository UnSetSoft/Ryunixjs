...
| CLI                           | `yargs`, `chalk`, `boxen`; binario `ryunix` → `webpack/bin/index.js` (fuente `.ts`)            |
...
| `pnpm run typecheck`    | Turbo: `tsc --noEmit` por paquete (ver [guía TypeScript](./typescript-en-el-monorepo.md)) |
