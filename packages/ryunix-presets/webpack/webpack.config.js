...
import { getPackageManager, ENV_HASH, getEnviroment, resolveApp, } from './utils/index.js';
import fs from 'fs';
...
const sharedWebpackConfig = {
    ...
    resolve: {
        alias: config.webpack.resolve.alias && getAlias(config.webpack.resolve.alias),
        extensions: [
            '.js',
            '.jsx',
            '.ryx',
...
const getPlugins = (isServer = false) => [
    ...
    new webpack.DefinePlugin({
        ...
        'process.env.RYUNIX_SSR': JSON.stringify(config.ssr || (config.legacy.ssg?.prerender?.length ?? 0) > 0),
                ...
                if (req.method === 'GET' && req.url.startsWith('/_ryunix/source')) {
                    try {
                        ...
                        if (!filePath || !lineStr) {
                            ...
                            return res.end(JSON.stringify({ error: 'Missing file or line parameter' }));
                        }
