...
const resolveApp = (appDirectory, relativePath) => resolve(appDirectory, relativePath);
function getPackageManager() {
    ...
    if (!agent) {
        ...
        if (parent.endsWith('pnpx') || parent.endsWith('pnpm'))
            return 'pnpm';
        if (parent.endsWith('bunx') || parent.endsWith('bun'))
            return 'bun';
...
const getEnviroment = () => Object.keys(process.env)
...
async function cleanCacheDir(dirPath) {
    try {
        ...
        logger.info(`webpack cache cleaned ${chalk.bold(chalk.green('successfully'))}`);
    }
    catch (err) {
...
async function cleanBuildDirectory(dirPath) {
    try {
        await fs.access(dirPath);
        await fs.rm(dirPath, { recursive: true, force: true });
        logger.info(`static folder cleaned ${chalk.bold(chalk.green('successfully'))}`);
    }
    catch (err) {
        // Directory does not exist or some error occurred
        if (err.code === 'ENOENT') {
...
export function convertFlatToClassic(configArray) {
    ...
    for (const cfg of configArray) {
        ...
        // Combinar env
        if (cfg.env)
            Object.assign(combined.env, cfg.env);
        // Combinar globals
        if (cfg.globals)
            Object.assign(combined.globals, cfg.globals);
        ...
        // Combinar reglas
        if (cfg.rules)
            Object.assign(combined.rules, cfg.rules);
        // Parser
...
export { getPackageManager, ENV_HASH, getEnviroment, resolveApp, RYUNIX_APP, getPackageVersion, cleanCacheDir, cleanBuildDirectory, };
