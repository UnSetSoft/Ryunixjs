...
class RyunixRoutesPlugin {
    ...
    extractFrontmatter(content, baseDir) {
        ...
        const importRegex = /import\s+\w+\s*,\s*\{[\s\S]*?frontmatter\s+as\s+(\w+)[\s\S]*?\}\s+from\s+["']([^"']+\.mdx)["']/g;
        let match;
    ...
    parseRoutes(content) {
        ...
        while ((match = routeRegex.exec(content)) !== null) {
            ...
            const pathMatch = routeBlock.match(/path:\s*([`"'][^`"']*[`"'](?:\s*\|\|\s*[`"'][^`"']*[`"'])?)/);
            if (!pathMatch)
                continue;
            ...
            if (noRenderLinkMatch) {
                const noRenderValue = this.evaluateExpression(noRenderLinkMatch[1].trim());
