...
import { getConfig } from './settingfile.js'
import { normalizeStyleConfig } from './styleConfig.js'

// eslint-plugin-react may not be installed - handle gracefully
let reactPlugin: unknown
try {
  const loadedReactPlugin = await import('eslint-plugin-react')
  reactPlugin = loadedReactPlugin.default ?? loadedReactPlugin
} catch (_e) {
  console.warn(
...
const userConfig = getConfig() as Record<string, unknown>

/**
 * Check if a config path exists in userConfig and warn if it's deprecated.
 */
const warnDeprecated = (configPath: string, message: string) => {
 ...
 * @param keyPath - Dot notation path (e.g., 'webpack.production')
 * @param defaultValue - Fallback value
 * @returns Config value or default
 */
const getConfigValue = <T>(keyPath: string, defaultValue: T): T => {
...
const defaultSettings = {
  ...
  style: normalizeStyleConfig(getConfigValue('style', false)),
  ...
  favicon: getConfigValue<boolean | string>(
    'favicon',
    getConfigValue<boolean | string>('static.favicon', true),
  ),
  compiler: getConfigValue('compiler', 'swc'),
  ...
  legacy: {
    ...
    template: getConfigValue<string | false>('static.customTemplate', false),
    ssg: {
      prerender: getConfigValue<string[]>('experimental.ssg.prerender', []),
      sitemap: {
        enable: getConfigValue('experimental.ssg.sitemap.enable', false),
        baseURL: getConfigValue<string | false>(
          'experimental.ssg.sitemap.baseURL',
          false,
        ),
