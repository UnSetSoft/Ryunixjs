    ...
    scanDirectory(dir, basePath) {
        ...
        for (const entry of entries) {
            if (entry.isFile()) {
                ...
                if (name === 'layout')
                    layout = assign(layout, fullPath);
                else if (name === 'index')
                    index = assign(index, fullPath);
                else if (name === 'error')
                    errorFile = assign(errorFile, fullPath);
                else if (name === 'loading')
                    loadingFile = assign(loadingFile, fullPath);
            }
        }
        for (const entry of entries) {
