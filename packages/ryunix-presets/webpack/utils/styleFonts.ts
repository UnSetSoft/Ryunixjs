...
const FONT_PRESETS: Record<string, RyunixStyleFontResolved> = {
  system: {
    id: 'system',
    family: null,
    links: [],
...
function buildFontHeadLinks(font: RyunixStyleFontResolved | undefined): string {
  if (!font?.links?.length) return ''
  return font.links.join('\n    ')
}

function buildFontCss(font: RyunixStyleFontResolved | undefined): string {
