...
const extractSSGRoutes = (routes) => {
    ...
    const processRoute = (route, parentPath = '') => {
        ...
        if (!route.path || route.path.includes(':'))
            return;
        if (route.NotFound || route.noRenderLink)
            return;
...
const generateMetaTags = (meta, defaultMeta = {}) => {
    ...
    const addMetaTag = (key, value) => {
        if (!value || INTERNAL_KEYS.has(key))
            return;
        ...
        if (Array.isArray(value)) {
            ...
            if (content)
                lines.push(`<meta ${attr}="${escapedKey}" content="${escapeHtml(content)}" />`);
        }
        else if (value) {
...
const buildSSG = async (routesConfig, config, buildDir, debug = false) => {
    ...
    if (routes.length === 0) {
        if (debug)
            console.log('[SSG Debug] No valid routes found to prerender.');
        return;
    }
    ...
    const copyStaticIfExist = (src, dest) => {
        ...
        if (fs.existsSync(fullSrc)) {
            ...
            if (debug)
                console.log(`[SSG] ✅ ${dest} copied from ${src}`);
            return true;
        }
        return false;
    };
            ...
            try {
                ...
                if (typeof manifestFn === 'function') {
                    ...
                    if (data && typeof data === 'object') {
                        ...
                        if (debug)
                            console.log('[SSG] ✅ manifest.json created from .js');
                        hasManifest = true;
                    }
                }
            }
    ...
    if (sitemapFilePath) {
            ...
            if (typeof sitemapFn !== 'function') {
                ...
                if (typeof generateSitemaps === 'function') {
                    ...
                    if (baseURL && segments.length > 0) {
                        ...
                        if (debug)
                            console.log(`${chalk.green('✔')} Sitemap:  ${chalk.bold('Generated Index')} (${segments.length} sitemaps)`);
                    }
                }
                else {
                    // ── Single sitemap mode ─────────────────────────────────────────
                    ...
                    if (Array.isArray(entries) && entries.length > 0) {
                        ...
                        if (debug)
                            console.log(`${chalk.green('✔')} Sitemap:  ${chalk.bold('Generated')} (${entries.length} URLs)`);
                    }
                    else {
        ...
        try {
            ...
            if (!baseURL) {
                ...
                fs.writeFileSync(path.join(buildDir, 'static', 'sitemap.xml'), xml);
                if (debug)
                    console.log(`${chalk.green('✔')} Sitemap:  ${chalk.bold('Generated (Legacy)')}`);
            }
        }
        catch (error) {
    ...
    if (robotsFilePath) {
        try {
            ...
            if (typeof robotsFn !== 'function') {
                ...
                if (debug)
                    console.log(`${chalk.green('✔')} Robots:   ${chalk.bold('Generated')}`);
            }
        }
        catch (e) {
        ...
        if (baseURL) {
            try {
                ...
                fs.writeFileSync(path.join(buildDir, 'static', 'robots.txt'), robotsTxt);
                if (debug)
                    console.log(`${chalk.green('✔')} Robots:   ${chalk.bold('Generated (Legacy)')}`);
            }
            catch (error) {
...
export { extractSSGRoutes, generateSitemap, generateRobotsTxt, generateMetaTags, prerenderRoute, buildSSG, importEsmFile, };
