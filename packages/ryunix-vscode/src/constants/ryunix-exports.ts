...
export const RYUNIX_EXPORT_DOCS: Record<string, string> = {
  ...
  useReducer:
    'Estado con reducer: `useReducer(reducer, initialArg, init?)`.',
  useEffect:
    'Efecto después del commit. Segunda argumento opcional: array de dependencias.',
  useLayoutEffect: 'Como `useEffect`, ejecutado de forma síncrona tras mutaciones DOM.',
  ...
  RouterProvider: 'Proveedor del router de Ryunix; envuelve el árbol de la app.',
  ...
  generateMetadata:
    'Función async que devuelve metadatos dinámicos para SSR.',
}
