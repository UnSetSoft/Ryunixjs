        ...
        if (state.isHydrating && state.hydrateCursor) {
            ...
            if (isText || isElement) {
                ...
                if (process.env.NODE_ENV !== 'production') {
                    console.warn(`[Hydration] Mismatch at ${getTypeLabel(fiber.type)}. Expected ${domNode.nodeType === 1 ? domNode.tagName : 'text'} but got ${String(fiber.type)}. Falling back to CSR.`);
                }
...
export { updateFunctionComponent, updateHostComponent, MDXContent, MDXProvider, useMDXComponents, getMDXComponents, defaultComponents, Image, };
