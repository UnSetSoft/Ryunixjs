...
chrome.runtime.onMessage.addListener((msg) => {
    if (msg.source !== 'ryunix-devtools' || !msg.payload)
        return;
...
function renderTree() {
    ...
    tree.innerHTML = fibers
        .map((f, i) => `<div class="tree-item" data-index="${i}">
...
function updatePerformance() {
    ...
    if (!totalComponents || !totalRenders || !avgTime || !slowList)
        return;
    ...
    if (slow.length > 0) {
        slowList.innerHTML = slow
            .map((f) => `<div class="slow-component">
